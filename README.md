...
- [Getting Started](#-getting-started)
  ...
  - [Method 1: Local Development](#method-1-running-normally-local-development)
...
- **Advanced AI & Semantic Vector Search:**
  ...
  - **FAISS Vector Store** for high-performance local vector similarity search, enabling deep semantic retrieval across uploaded documents and media transcripts.
...
- **Redis Semantic Response Caching:** Uses Redis to store semantic cache entries with configurable TTLs. Incoming user queries are converted to vector embeddings and compared against cached queries using cosine similarity. If a query achieves ≥95% semantic similarity to a cached entry, the system instantly streams the cached AI response, bypassing the LLM entirely to eliminate latency and reduce API costs.
- **API Rate Limiting:** Authentication endpoints are protected with a rate limiter to prevent brute-force and abuse.

### Frontend (React 19 + TypeScript + Vite + Tailwind CSS)

- **Premium UI:** Built with Tailwind CSS, Framer Motion, and Aceternity/Shadcn UI (Background Ripple, Text Generate effects, Placeholders/Vanish inputs).
- **Interactive Chat Management:** Collapsible sidebar for organizing, renaming, deleting, and navigating chat sessions with pagination.
- **Clickable Timestamp Citations:** Audio/video citations render as timestamp pills (e.g., `[0:05 - 1:52]`) that, when clicked, seek a `plyr-react` media player to the exact second and begin playback.
- **Robust Session Maintenance:** Custom `useSilentTokenRefresh` hook refreshes authentication tokens in the background without interrupting user workflows.
...
Both operations run concurrently, so the user reaches a ready state as fast as possible.
...
### Method 1: Running Normally (Local Development)
...
### Method 2: Running with Docker Compose (Production / Containerized)

1. Ensure `backend/.env` and `frontend/.env` are fully configured.
...
- `insightflow-backend` — FastAPI server on port `8000`.
- `insightflow-frontend` — Nginx serving the optimized React build on port `3000`.
...
For detailed information on all backend endpoints, required request payloads, expected responses, and success/error scenarios, please refer to the complete [API Documentation](API.md).
...
> The frontend token TTL should be slightly shorter than the backend `ACCESS_TOKEN_TTL` so the `useSilentTokenRefresh` hook can preemptively refresh tokens before they expire.

---

## 🧪 Running Tests

InsightFlow includes a comprehensive suite of pytest-powered unit and integration tests covering authentication, rate limiting, file upload handling, cancellation workflows, semantic caching, and AI service utilities.

### 1. Running the Test Suite Locally

To run the entire test suite, ensure your virtual environment is active and execute pytest from the `backend` directory:
...
### 2. Running with Detailed Output and Coverage

To view detailed test execution progress along with a complete code coverage report:
